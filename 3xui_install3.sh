...
if [[ "$DO_SWAP" == "1" ]]; then
	...
	if [[ -n "$(swapon --show --noheadings 2>/dev/null)" ]]; then
		...
		bad "на диске меньше $((SWAP_MB + 2048)) МБ — свап не завожу, сервер работает и без него"
	elif { fallocate -l ${SWAP_MB}M /swapfile 2>/dev/null || dd if=/dev/zero of=/swapfile bs=1M count=${SWAP_MB} status=none; } \
		&& chmod 600 /swapfile && mkswap /swapfile >/dev/null 2>&1 && swapon /swapfile 2>/dev/null; then
		grep -q '^/swapfile ' /etc/fstab || echo '/swapfile none swap sw 0 0' >> /etc/fstab
		ok "свап ${SWAP_MB} МБ включён (/swapfile, переживёт перезагрузку)"
	else
		rm -f /swapfile
		bad "свап включить не удалось (виртуализация без свапа?) — сервер работает и без него"
	fi
...
# Оставляем две проверки подряд в часы, когда дома спят: 23 и 00 UTC = 04 и 05 утра Екатеринбурга
# (вторая — повтор на случай сбоя первой; если первая продлила, вторая ничего не делает).
...
if crontab -l 2>/dev/null | grep -q 'acme.sh --cron'; then
	...
	RENEW_HOUR_RETRY=$(( (RENEW_HOUR_LOCAL + 1) % 24 ))
	crontab -l | sed -E "/acme\.sh --cron/ s/^([0-9]+) [0-9,*]+ /\1 ${RENEW_HOUR_LOCAL},${RENEW_HOUR_RETRY} /" | crontab -
	ok "проверка сертификата — в ${RENEW_HOUR_UTC}:xx UTC и повтор через час"
fi
